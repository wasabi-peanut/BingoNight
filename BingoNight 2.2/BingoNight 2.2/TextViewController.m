...
-(void)createContentArrays{
    ...
    _arrayContentIntro = [[NSMutableArray alloc] initWithObjects:
                          @"Welcome to My Bingo Event, the only app you need to host your Bingo event or just to host a Bingo game in your home, business, classroom or anywhere. These instructions are divided by the options you'll find when you open the game.\n\nThe game is ready to play if you hit PLAY. But let your creative juices flow. Go to SETUP to customize just about every aspect of the experience.\n\nOnce in SETUP, use LOCAL SETTINGS to adjust settings for individual games. Use GLOBAL SETTINGS to make changes to impact all the games, such as the name of the event and how to choose and display the selected ball.\n\nYou can play right away. But if you're going for the special My Bingo Event, we suggest you take some time to test your settings, espcially if you plan to use a projector, which can sometimes distort colors.\n\nWe recommend connecting your iPad to a projector, large screen TV or several monitors for the best My Bingo Event experience.\n\nVisit us at www.AandADevelopment.com",
                          nil];
    _arrayContentGamePlay = [[NSMutableArray alloc] initWithObjects:
                             @"The choose game screen lets you pick which game to play. Use SETUP to set the number of games up to 12. If you have activated a raffle, a raffle ticket will appear to let you display the prizes. If you have a theme song, music play and stop buttons appear. Each button is visible only when each option is selected. Again, use SETUP to activate these options.\n\nAlso see RAFFLE and THEME SONG",
                             ...
                             @"Once you have selected the game to play, you will go to the Game Play Screen. The page shows the traditional Bingo numbers and the grid displaying the game version. It also shows the name of the game. There is an area to display a game message. This area will have the same color scheme as the number grid. It will only appear if there is a message.\n\nAll of these features, including colors, grid designs and text can be customized in SETUP and LOCAL SETTINGS.",
                             @"Once in the Game Play Screen, My Bingo Event gives you the option to choose a ball within the app or to use a separate ball selector. The default is to use the chooser within the app. To choose a ball, press Choose Ball in the top menu bar. After the ball is displayed, the CHOOSE BALL option reappears.\n\nIf you prefer to use a ball selector outside of the app, go to SETUP and GLOBAL SETTINGS and deselect USE AUTO SELECT. To choose a ball without AUTO SELECT, simply press the number square of the selected number. If you make a mistake, hit the UNDO button.\n\nSee CHOOSE BALL in GLOBAL SETTINGS",
                             @"When somebody shouts Bingo, it's time to check the cards. Press the CHECK CARD button on the top row to pause the game. The default is to use special checking, which means a song and an annimation will appear -- and occupy the audience while the checking is underway.\n\nIf you turn off special checking, then a simple sign flashes on the screen. Go to SETUP and GLOBAL SETTINGS to select special checking. You will be able to customize the music and animation too.\n\nIf it turns out there was no Bingo and the game continues, hit either return to game or the Back button (depending on whether you are using special checking.) Resume selecting numbers.",
                             @"If you determine that a player or players have Bingo, hit the Winner button in the top menu. To avoid errors, you must hit Confirm Winner button on the right side. If you are using special checking, you will go to an animated winner screen, with a sound of your choice. If not, a simple sign saying Winner will appear. You can then use the Return to Game and End Game options to get to the Start Screen.",
...
nil];
    _arrayContentSetupLocal = [[NSMutableArray alloc] initWithObjects:
                             ...
                             @"Use the plus and minus buttons to choose the number of games, up to 12 games. Added games use the default display but you can later make changes. If you reduce the number of games, you will lose the settings for eliminated games.",
                               //COLOR SCHEMES
                             @"You can choose what color scheme you want to use for each game. There are several preset schemes to choose from. You cannot change these schemes. Varying color schemes by game adds more style to your Bingo event. You can also coordinate with the color of game cards.\n\nIf you want to edit a scheme or create a new one, hit the plus button. You will give the scheme a name and be able to edit each aspect. When you add a new scheme, it uses the current scheme as the base. Therefore, if you want to make an edit to the default Red scheme, add a new scheme while Red is selected. Also note that the markers are tied to the scheme. If you want to change the markers used, you must create a new scheme. Again start with the base that is closest to what you want to use.\n\nYou can customize most aspects by adjusting the color sliders. All changes are saved. Adjust the dial to the aspect you want to change and go for it. For example, to change the background color, choose that attribute. The other choices are the background and text colors of the grid. Called box and text refers to the colors after the number has been announced. You often will want these to have a faded or darkened look so players can easily distinguish between what has been called and what is left.\n\nYou can also change the Bingo Letters, which are the letters B-I-N-G-O that appear aside the number rows in the grid. Remember to distinguish these from the background color.Your changes will display in the sample grid on this page.\n\nYou can also change the name of the game. You will see a yellow box around the display, which means editing is enabled. The name appears above the grid on the game play display. It is a good place to put the name or number of game and perhaps the type of game to play. The text will shrink to fit the space but if you type too much, it may be hard to see the words when displayed.\n\nIf you want longer comments, consider using the optional comment section on this page. See GAME COMMENTS for instructions.",
                               //GRIDS
                             @"You can select the game type or GRID for each game. For example, Game One may require the traditional five in a row, either up, down or across to Bingo. Game Two could require an X pattern or a Perimeter pattern. There are several defaults that cannot be changed. If you want add a new pattern, then press the plus button, name the grid and add it. As with the color schemes, your new grid is based on the grid that is selected. See CUSTOMIZE GRID.\n\nNote that the markers are not part of the grid but part of the color scheme. To change the markers, edit the color scheme. See COLOR SCHEMES.",
                             //CUSTOMIZE GRIDS
    ...
    _arrayContentSetupGlobal = [[NSMutableArray alloc] initWithObjects:
                                //Event Screen
                             @"The Event Screen allows you to set up the name of the event. This name will appear on the start screen before you select which game to play or, if available, you go to the raffle display.\n\nTo enter the text, simply tap in the screen and write the text. With the controls on the bottom, you can choose the color for the text and for the background. Simply pick either option and adjust the color sliders. All color changes are saved. The text is saved when you dismiss the keyboard.\n\nYou can also choose the font style and color from the selectors. Use the line spacing slider to adjust the spacing between lines. Use the alignment choices to select left, center or right alignment.\n\nTip: If the font size is too large, the second and subsequent lines may disappear below the screen. Adjust the font size and line spacing to bring them back into view. Stray returns or lines at the end of the text may also cause text to disappear. Be sure to delete those if the words disappear from view before reaching the bottom of the screen",
                                //ADD IMAGE
                             @"You can also add an image to your title by tapping Get Image. You will be able to access images stored on your iPad, after you grant permission. If you don't grant permission but later change your mind, you can update your choice in the Settings function on the iPad.\n\nAfter you have selected the image, use the sliders to position and size it. You can also switch between having the image under the text and have the image over the text, using the under and over buttons. To remove image, simply press remove image\n\nTip: You can only add one image but you can combine different images and words into one image using an image editing program. You can then import your edited image.\n\nThe image is not stored within the App but remains on you iPad. Be sure not to delete the image from storage in your camera or photos if you intend to use it on the title screen",
                                //AUTO SELECT
                             @"If you choose to Use Auto Select, then My Bingo Event will automatically choose a ball during the game. If this button is switched off, then you must provide your own way to choose balls. You will still use My Bingo Event to display the called numbers and fpr other asepcts of the game.",
                                //SMART CHOICE
                             @"Using Smart Ball Choice is helpful when Auto Select is on. With Smart Ball Choice, only balls from relevant columns are selected. For example, if you play Four Corners, only balls from the B column and O column matter. The Auto Select will not select balls that begin with I, N or G. Smart Ball Choice has no impact if you are not using Auto Select",
                                //SPECIAL CHECKING
                             @"One feature of My Bingo Event is the ability to add animation and music to the perod while evaluating a potentially winning card. Choosing Special Checking gives you this option to add a little flavor between games. (Without Special Checking, a simple sign that says \"Hold Your Cards\" flashes on the game screen.\n\nIf you select Use Special Checking, press SET UP to go to the customization screen\n\nYou will be able to choose a checking pattern for each games. A sample of the animation appears below. You can also choose background music that matches this pattern. Finally, you can choose a sound that plays when you go to the Winner Screen. Press the play button to sample the music and sounds.\n\nYou can use your own music for checking music. Press ADD OWN SONGS and select your music from your iTunes account. Your own songs will appear at the end of the SONG CHOICES SELECTOR. Choose one of your own songs as you customize a game.\n\nTIP: If you choose your own songs, be sure they are downloaded to your iTunes account or, if you are streaming music through Apple Music, that you are connected to the internet on the night of your event. If the app cannot find the selected song, no sound will play.\n\nAlso, think about how long each checking period will take when you choose a song and checking animation. For example, checking if a card had a row of five takes much less time then checking if a perimeter or the entire card is filled. Choose the appropriate checking pattern based on how long you think it will appear on the screen.",
                                ...
                                //RAFFLE
                                @"Often, Bingo events have raffles. My Bingo Event allows you to have a raffle as part of the display. Use the HAVE A RAFFLE switch and press SETUP to enter the names of prizes. You can use ADD to add a new entry. Type as many lines as you'd like to name and describe the prize. The font size will adjust. Use EDIT to delete items and to change the order.\n\nTip: While the display will adjust to the display the description, be sure it remains large enough to be seen. Go to Play and hit the Raffle Ticket icon to have a look. The best way to utilize this feature is to put the items in the order that will be raffled off. That way, you can switch from item to item easily between drawings",
                                //MAKE CARDS
                                @"You can print your own Bingo cards with My Bingo Event. In the square next to the MAKE CARDS button, enter the number of pages to print. Then press MAKE CARDS. There are four cards per page.",
                                //THEME SONG
